...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.aktion.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.aktions.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('text') ? 'is-invalid' : '' }}" name="text_id" id="text_id">
                    @foreach($texts as $id => $entry)
                        <option value="{{ $id }}" {{ old('text_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('text'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('targets') ? 'is-invalid' : '' }}" name="targets[]" id="targets" multiple required>
                    @foreach($targets as $id => $target)
                        <option value="{{ $id }}" {{ in_array($id, old('targets', [])) ? 'selected' : '' }}>{{ $target }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ort') ? 'is-invalid' : '' }}" name="ort_id" id="ort_id">
                    @foreach($orts as $id => $entry)
                        <option value="{{ $id }}" {{ old('ort_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('ort'))
