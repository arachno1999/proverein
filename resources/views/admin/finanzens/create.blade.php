...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.finanzen.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.finanzens.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="datum">{{ trans('cruds.finanzen.fields.datum') }}</label>
                <input class="form-control date {{ $errors->has('datum') ? 'is-invalid' : '' }}" type="text" name="datum" id="datum" value="{{ old('datum') }}" required>
                @if($errors->has('datum'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('betrag') ? 'is-invalid' : '' }}" type="number" name="betrag" id="betrag" value="{{ old('betrag', '') }}" step="0.01" required>
