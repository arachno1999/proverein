...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.verein.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.vereins.update", [$verein->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.verein.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $verein->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('beschreibung') ? 'is-invalid' : '' }}" name="beschreibung" id="beschreibung">{!! old('beschreibung', $verein->beschreibung) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('gruendung') ? 'is-invalid' : '' }}" type="text" name="gruendung" id="gruendung" value="{{ old('gruendung', $verein->gruendung) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('register') ? 'is-invalid' : '' }}" type="text" name="register" id="register" value="{{ old('register', $verein->register) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tags') ? 'is-invalid' : '' }}" name="tags[]" id="tags" multiple>
                    @foreach($tags as $id => $tag)
                        <option value="{{ $id }}" {{ (in_array($id, old('tags', [])) || $verein->tags->contains($id)) ? 'selected' : '' }}>{{ $tag }}</option>
                    @endforeach
                </select>
                @if($errors->has('tags'))
