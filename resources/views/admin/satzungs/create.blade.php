...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.satzung.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.satzungs.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="paragraph">{{ trans('cruds.satzung.fields.paragraph') }}</label>
                <input class="form-control {{ $errors->has('paragraph') ? 'is-invalid' : '' }}" type="number" name="paragraph" id="paragraph" value="{{ old('paragraph', '') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('inhalt') ? 'is-invalid' : '' }}" name="inhalt" id="inhalt">{!! old('inhalt') !!}</textarea>
