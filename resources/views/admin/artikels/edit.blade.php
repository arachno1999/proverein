...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.artikel.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.artikels.update", [$artikel->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('bezeichnung') ? 'is-invalid' : '' }}" type="text" name="bezeichnung" id="bezeichnung" value="{{ old('bezeichnung', $artikel->bezeichnung) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('menu') ? 'is-invalid' : '' }}" name="menu_id" id="menu_id" required>
                    @foreach($menus as $id => $entry)
                        <option value="{{ $id }}" {{ (old('menu_id') ? old('menu_id') : $artikel->menu->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('menu'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('sichtbar') ? 'is-invalid' : '' }}" type="text" name="sichtbar" id="sichtbar" value="{{ old('sichtbar', $artikel->sichtbar) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('end') ? 'is-invalid' : '' }}" type="text" name="end" id="end" value="{{ old('end', $artikel->end) }}">
                @if($errors->has('end'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('intro') ? 'is-invalid' : '' }}" name="intro" id="intro">{!! old('intro', $artikel->intro) !!}</textarea>
                @if($errors->has('intro'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('fulltext') ? 'is-invalid' : '' }}" name="fulltext" id="fulltext">{!! old('fulltext', $artikel->fulltext) !!}</textarea>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Artikel::POSITION_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('position') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="position_{{ $key }}" name="position" value="{{ $key }}" {{ old('position', $artikel->position) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('reihenfolge') ? 'is-invalid' : '' }}" type="number" name="reihenfolge" id="reihenfolge" value="{{ old('reihenfolge', $artikel->reihenfolge) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('template') ? 'is-invalid' : '' }}" name="template_id" id="template_id" required>
                    @foreach($templates as $id => $entry)
                        <option value="{{ $id }}" {{ (old('template_id') ? old('template_id') : $artikel->template->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('submenu') ? 'is-invalid' : '' }}" name="submenu_id" id="submenu_id">
                    @foreach($submenus as $id => $entry)
                        <option value="{{ $id }}" {{ (old('submenu_id') ? old('submenu_id') : $artikel->submenu->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
