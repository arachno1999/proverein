...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.mitglied.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.mitglieds.update", [$mitglied->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $mitglied->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('birthday') ? 'is-invalid' : '' }}" type="text" name="birthday" id="birthday" value="{{ old('birthday', $mitglied->birthday) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type_id" id="type_id" required>
                    @foreach($types as $id => $entry)
                        <option value="{{ $id }}" {{ (old('type_id') ? old('type_id') : $mitglied->type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('anrede') ? 'is-invalid' : '' }}" type="text" name="anrede" id="anrede" value="{{ old('anrede', $mitglied->anrede) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email" name="email" id="email" value="{{ old('email', $mitglied->email) }}">
                @if($errors->has('email'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('eintritt') ? 'is-invalid' : '' }}" type="text" name="eintritt" id="eintritt" value="{{ old('eintritt', $mitglied->eintritt) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('austritt') ? 'is-invalid' : '' }}" type="text" name="austritt" id="austritt" value="{{ old('austritt', $mitglied->austritt) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('phone') ? 'is-invalid' : '' }}" type="text" name="phone" id="phone" value="{{ old('phone', $mitglied->phone) }}">
                @if($errors->has('phone'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('anschrift') ? 'is-invalid' : '' }}" name="anschrift" id="anschrift">{{ old('anschrift', $mitglied->anschrift) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('organs') ? 'is-invalid' : '' }}" name="organs[]" id="organs" multiple>
                    @foreach($organs as $id => $organ)
                        <option value="{{ $id }}" {{ (in_array($id, old('organs', [])) || $mitglied->organs->contains($id)) ? 'selected' : '' }}>{{ $organ }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tags') ? 'is-invalid' : '' }}" name="tags[]" id="tags" multiple>
                    @foreach($tags as $id => $tag)
                        <option value="{{ $id }}" {{ (in_array($id, old('tags', [])) || $mitglied->tags->contains($id)) ? 'selected' : '' }}>{{ $tag }}</option>
                    @endforeach
                </select>
                @if($errors->has('tags'))
